...
\subsection{Вычислительный метод}
...
В результате работы была написана демонстрационная программа на языке $c++$. Программа позволяет исследовать вид волновых функций в сферической потенциальной яме, а также наблюдать вид фазовой функции.
...
Программа позволяет задавать ширину и высоту барьера, величину орбитального момента, границы моделирования. На двух графиках отображаются внешний вид фазовой функции и до шести волновых функций с соответствующими им уровнями энергий.

Рассмотрим некоторые результаты моделирования.

\subsection{Бесконечно глубокая сферическая потенциальная яма}

Бесконечно глубокая потенциальная яма получается при нулевой глубине барьера. Ширина ямы задается в этом случае шириной области моделирования. Волновая функция на границе ямы должна обращаться в нуль. Собственные уровни энергии положительны.

На \autoref{fig:inf_phi}-\autoref{fig:inf_l1} представлены результаты моделирования.

Из общей теории известно, что в случае сферической потенциальной ямы решением уравнения Шредингера для частицы с орбитальным моментом $l$ является функция Бесселя порядка $l$, $J_l$. Нули $J_l$ совпадают с границами ямы.

Результат моделирования для нулевого орбитального момента совпал с теоретическими доводами. Однако для остальных значений орбитального момента особенность уравнений в начале координат не позволила примененными вычислительными методами достичь устойчивого решения вблизи нуля (\autoref{fig:inf_l1a}). Подмена в этой области функций вида $r^{-1}$, входящих в расчетные уравнения, некоторой большой по величине константой частично решает проблему. Точность моделирования в окрестности нуля тем выше, чем меньше область \enquote{обрезания} функции $r^{-1}$, однако сужение этой области чревато сильной неустойчивостью при приближении к границам ямы, вызывает сильные осцилляции фазовой функции и т.д. Поэтому в окрестности нуля наблюдается один достаточно широкий \enquote{всплеск}. Разрешить волновые функции с ненулевым (фактически, максимум первым-вторым) орбитальным моментом можно в режиме повышения точности в окрестности нуля (\autoref{fig:inf_l1}), однако при этом также возникают некоторые искажения их формы -- \enquote{сжатие} к началу координат.

Также проблематично наблюдать низкие энергетические уровни (нулевой, первый, ...) в силу сильной нестабильности как фазовой, так и волновых функций.

\enquote{Ступеньки} фазовой функции в яме выражены слабо.

\begin{figure}[!htb]%
\centering
\includegraphics[width=1\textwidth]{inf_phi}%
\caption[Вид фазовой функции.]{Вид фазовой функции для частицы с нулевым орбитальным моментом}%
\label{fig:inf_phi}%
\end{figure}
...
\subfloat[][]{%
\label{fig:inf_0}%
\includegraphics[width=0.45\textwidth]{inf_0}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_1}%
\includegraphics[width=0.45\textwidth]{inf_1}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_2}%
\includegraphics[width=0.45\textwidth]{inf_2}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_3}%
\includegraphics[width=0.45\textwidth]{inf_3}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_4}%
\includegraphics[width=0.45\textwidth]{inf_4}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_5}%
\includegraphics[width=0.45\textwidth]{inf_5}}%
\hspace{8pt}%
%
\caption[Бесконечно глубокая потенциальная яма.]{Бесконечно глубокая потенциальная яма. Орбитальный момент равен нулю.
\subref{fig:inf_0}-\subref{fig:inf_5} соответствуют нулевому-пятому энергетическим уровням} %
\label{fig:inf_l0}%
\end{figure}
...
\subfloat[][]{%
\label{fig:inf_l1_4a}%
\includegraphics[width=0.45\textwidth]{inf_l1_4a}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_l1_6a}%
\includegraphics[width=0.45\textwidth]{inf_l1_6a}}%
\hspace{8pt}%
%
\caption[Бесконечно глубокая потенциальная яма.]{Бесконечно глубокая потенциальная яма. Орбитальный момент равен единице.
\subref{fig:inf_l1_4a}-\subref{fig:inf_l1_6a} соответствуют четвертому и шестому энергетическим уровням} %
\label{fig:inf_l1a}%
\end{figure}
...
\subfloat[][]{%
\label{fig:inf_l1_4}%
\includegraphics[width=0.45\textwidth]{inf_l1_4}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:inf_l1_6}%
\includegraphics[width=0.45\textwidth]{inf_l1_6}}%
\hspace{8pt}%
%
\caption[Бесконечно глубокая потенциальная яма.]{Бесконечно глубокая потенциальная яма. Орбитальный момент равен единице. Уточненная область в окрестности нуля.
\subref{fig:inf_l1_4}-\subref{fig:inf_l1_6} соответствуют четвертому и шестому энергетическим уровням} %
\label{fig:inf_l1}%
\end{figure}

\subsection{Конечная сферическая потенциальная яма}

В случае конечной ямы все сказанное выше остается в силе за исключением того, что волновые функции теперь могут \enquote{просачиваться} за границы ямы (\autoref{fig:fin_l0}-\autoref{fig:fin_l1}). Область моделирования при этом берется в несколько раз больше ширины ямы.

\enquote{Ступеньки} фазовой функции видны отчетливо.
...
\subfloat[][]{%
\label{fig:fin_l0_03}%
\includegraphics[width=0.45\textwidth]{fin_l0_03}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:fin_l0_phi}%
\includegraphics[width=0.45\textwidth]{fin_l0_phi}}%
\hspace{8pt}%
%
\caption[Конечная яма.]{Конечная яма. Орбитальный момент равен нулю. Вертикальными линиями обозначены границы ямы.
\subref{fig:fin_l0_phi} Фазовая функция \subref{fig:fin_l0_03} Волновые функции уровней 0-3} %
\label{fig:fin_l0}%
\end{figure}
...
\subfloat[][]{%
\label{fig:fin_l1_14}%
\includegraphics[width=0.45\textwidth]{fin_l1_14}}%
\hspace{8pt}%
%
\subfloat[][]{%
\label{fig:fin_l1_phi}%
\includegraphics[width=0.45\textwidth]{fin_l1_phi}}%
\hspace{8pt}%
%
\caption[Конечная яма.]{Конечная яма. Орбитальный момент равен единице. Вертикальными линиями обозначены границы ямы.
\subref{fig:fin_l1_phi} Фазовая функция \subref{fig:fin_l1_14} Волновые функции уровней 1-4} %
\label{fig:fin_l1}%
\end{figure}

\section{Выводы}

В ходе работы была разработана демонстрационная программа для исследования вида волновых функций в сферической потенциальной яме. Программа также позволяет исследовать влияние параметров ямы (высота, ширина, форма) в ручном режиме.

При помощи программы были исследованы бесконечная и конечная ямы. Получен вид волновых функций при различных входных параметрах.

Также была отмечена сложность моделирования задачи ввиду появления в уравнениях особенности в начале координат, вносимой членом, пропорциональным орбитальному моменту.
